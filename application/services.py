...
from .models import Elocution, Intent
...
class Interpreter:
    ...
    def translate(self, elocution_text):
        if len(elocution_text) > 0:
            ...
            print(cache.get(self._chat_id))

            # self.pre_process_entry(elocution_text)
            response_text = ''
            predictor = Predictor()
    ...
    def get_response(self, intent):
        ...
        if intent.response is not None:
            ...
            if intent.feature is not None:
                ...
                try:
                    feature_class = getattr(features, intent.feature.call)
                except AttributeError:
                    self._logger.error('The response feature class can not '
                                       'be found in the features module')
                    sys.exit('Bad girl!')
                feature = feature_class(intent)
                response_text = feature.handle()
            else:
                try:
                    response = intent.response
                    response_text = response.get_response_elocution()
                except ObjectDoesNotExist:
                    self._logger.error('The intent object cannot find a '
                                       'response object')
                    sys.exit('Bad girl!')
